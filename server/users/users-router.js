      ...
      if (user) {
        Posts.findBy({ user_id: user.id })
          ...
          .catch(err => {
            res
              .status(401)
              .json({ message: `Failed To Find Posts For: ${user.username}` });
          });
      } else {
        res
          .status(401)
          .json({ message: `Failed To Find User With ID: ${req.params.id} ` });
      }
    })
    .catch(err => {
    ...
    .then(user => {
      if (user && bcrypt.compareSync(updatedUser.password, user.password)) {
        Users.update(updatedUser, id).then(updated => {
          res
            .status(200)
            .json({ success: "updated", id: user.id, updated: updatedUser });
        });
      } else if (
        user &&
        !bcrypt.compareSync(updatedUser.password, user.password)
      ) {
    ...
    .then(user => {
      if (user) {
        Users.remove(req.params.id)
          ...
          .catch(err => {
            res.status(500).json({
              message: `Failed To Delete User w/ ID: ${req.params.id}`
            });
          });
      } else {
        res
          .status(401)
          .json({ message: `Could not Find User w/ ID: ${req.params.id}` });
      }
    })
    .catch(err => {
