...
describe('Post-Router Endpoints', () =>{
    ...
    test('PUT to /api/posts/:id', async () =>{
        ...
        //does it return the expected data format?
        expect(posts.type).toMatch(/json/)

        const res = await request(server)
        //declare endpoint and edit last post in posts array
        .put(`/api/posts/${posts.body[posts.body.length-1].id}`)
        //set token to header
        .set("authorization", token)
        //send new post body
        .send({
    ...
    test('DELETE to /api/posts/:id', async () =>{
        ...
        const res = await request(server)
        //declare endpoint
        .delete(`/api/posts/${posts.body[posts.body.length-1].id}`)
        //set token to header
        .set("authorization", token)
        //does it return the expected status code?
        expect(res.status).toBe(200);
